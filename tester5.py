...
while True:
 ...
 for (x,y,w,h) in faces:
   ...
   cv2.rectangle(cap_image, (x,y), (x+w,y+h),(255,0,0),7)
   ...
   cv2.putText(cap_image, emotion_prediction, (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2 )
   
   current_time = datetime.now().replace(microsecond=0)

   stime = str(current_time-start_time)
   
   if f==1:
       sheet1.write(1,0,emotion_prediction)
       sheet1.write(1,1,stime)
       pi = 1
       p = emotion_prediction
       f = 2
       
   if f!=1:
       if emotion_prediction!=p:
           sheet1.write(pi,2,stime)
           sheet1.write(pi+1,0,emotion_prediction)
           sheet1.write(pi+1,1,stime)
           pi = pi+1
           p = emotion_prediction
